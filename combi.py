...
def xg_train(s,p):
    ...
    train=xgboost.DMatrix(xx,yy[s][p])
    ev=xgboost.DMatrix(xt,a[p])
    outfile = p+"."+str(s)+"_"+str(uniform(0.001,0.999))+".regressor"
    print("training "+p+str(s)+"------------------->"+d+"/"+outfile)
    r = regressor = xgboost.train({'max_depth':13, 'eta':0.1, 'verbosity':2 ,'subsample':0.5,'grow_policy':'lossguide','max_leaves':0}, train,1000, evals=[(ev,'eval')],early_stopping_rounds=5)
    pf,m,bt,cnt,pp,rpp,B = simulate5(r.predict(xgboost.DMatrix(xt)),a[p],p)
...
def nn_train(s,p):
    best_score=-9999999
    best_regressor=None
    r=regressor = MLPRegressor(warm_start=True,verbose=2,tol=1e-7,learning_rate='adaptive',hidden_layer_sizes=(len(x[0])),solver='sgd')
    outfile = p+"."+str(s)+"_"+str(uniform(0.001,0.999))+".regressor"
    print("training "+p+str(s)+"------------------->"+d+"/"+outfile)
    bj=0
...
def gb_train(s,p):
    ...
    for j in range(1,3000):
        ...
        r.fit(xx,yy[s][p])
        #score = r.score(xt,yt[s][p])
        #score = r2_score(yt[s][p],r.predict(xt))
        #score = my_score(yt[s][p],r.predict(xt))
        #score,m,bt,cnt,pp,rpp,B = simulate9(r.predict(xt),a[p],p,outfile,best_score)
        score,m,bt,cnt,pp,rpp,B = simulate(r.predict(xt),a[p],p)
        if score*abs(m)>best_score:
            bj=j
            bm=m
...
def gb_train_l2(c):
    ...
    SP = randint(2,13)
    SS = uniform(0.25,0.9)
    r=GradientBoostingRegressor(verbose=0, warm_start=True, loss='huber', learning_rate=lr, n_estimators=1,max_depth=9,min_samples_leaf=LV,min_samples_split=SP,subsample=SS)
    outfile = c+"."+"_"+str(LV)+"_"+str(SP)+"_"+str(SS)+".regressor2"
    print("training "+c+"------------------->"+d+"/"+outfile)
    bj=0
    ...
    for j in range(1,3000):
        ...
        r.fit(xx,yy[c])
        #score = r.score(xt,yt[s][c])
        #score = r2_score(yt[c],r.predict(xt))
        pf,m,wRate,cnt,pp,rpp,B = simulate(r.predict(xt),a[c],p)
        score = pf
        if score>best_score:
            bj=j
            best_score = score
...
def simulate5(lot,a,pair):
    ...
    for j in range(45):
        t = 0.1 + float(j)*.2
        pf,m,wRate,cnt,pp,rpp,B = simulate4(lot,a,p,t)
        ...
        if pf>mx:
            ...
            bB = B
            bm = m
    return mx,bm,bt,cnt,pp,rpp,bB            

def simulate7(lot,a,pair):
    mx = -999999999
    bB = None
    bm = 0
    debug = False
    ...
    for j in range(45):
        t = 0.1 + float(j)*.1
        pf,m,wRate,cnt,pp,rpp,B = simulate6(lot,a,p,t)
        if (debug): print(p+":"+str(t)+":   "+str(round(pf,4))+":"+str(cnt)+":"+str(round(m,3)))
        if cnt<0:break
        if pf>mx:
            mx = pf
            ...
            bB = B
            bm = m
    return mx,bm,bt,cnt,pp,rpp,B     

def simulate6(lot,a,pair,power):
    balance = 0
    ol = 0
    l = 0
    maxB = -999999
    ...
    for i in range(len(lot)):
        lt = lot[i]
        l = pow(float(abs(lt)), float(power))
        if lt<0: l=-l
        if (ol>0 and l>0 and l<ol) or (ol<0 and l<0 and l>ol): l = ol
        if abs(l)>0.000000001: tcnt+=1
        r=a[i]
        if (r<-990): continue        # rejects -999
        if (r>0 and l>0) or (r<0 and l<0):
            positive+=1
        ...
        ol = l
    cnt = len(lot)
    if maxDD==0 or cnt==0 or tcnt<20:
        return balance/1000,0.001,0,tcnt,0,0,B
    n = len(B)
    ...
    for i in range(n-1):
        if B[i+1]>B[i]: tg += B[i+1]-B[i]
        if B[i+1]<B[i]: tl += B[i]-B[i+1]
    if tl<0.000001:tl=0.000001
    pf = tg/tl   # profit factor
    return pf, (-100/-maxDD), float(pcnt)/cnt,tcnt,fdp(float(positive)/cnt),fdp(float(rpositive)/cnt),B

def simulate4(lot,a,pair,power=1):
    balance = 0
    ol = 0
    l = 0
    maxB = -999999
    ...
    for i in range(len(lot)):
        ...
        profit = (l * r - abs(l - ol) * spread[pair] / 2) * unit[pair] / value[pair[-3:]] - abs(l - ol) * commission[pair] / 2
        if profit > 0: pcnt+=1
        #if profit > 500: profit=500
        #if profit <-500: profit=-500
        balance += profit
        if balance > maxB: maxB = balance
        if maxB - balance > maxDD: maxDD = maxB - balance
        B.append(balance)
        if (ol<0 and l>0) or (ol>0 and l<0): switch+=1
        ol = l
    cnt = len(lot)
    if maxDD==0 or cnt==0 or tcnt<20:
        return balance/1000,0.001,0,tcnt,0,0,B
    ...
    for i in range(len(B)): B[i] = B[i]/B[-1]*100
    mg = (B[-1] - B[0]) / (len(B)-1)
    s = 0 # straightness
    for i in range(n): s += abs(mg*i+B[0]-B[i])
    ...
    #    dif = B[i] - B[i-1]
    #    if dif > 0: gain += dif**0.5
    #    else:       loss += abs(dif)**0.5

    #if loss==0: 
    #    loss=0.0000001
    #    print("echo division by zero")
    return n/s*(abs(balance)/balance), (-100/-maxDD), float(pcnt)/cnt,tcnt,fdp(float(positive)/cnt),fdp(float(rpositive)/cnt),B
    #return gain/loss, (-100/-maxDD), float(pcnt)/cnt,tcnt,fdp(float(positive)/cnt),fdp(float(rpositive)/cnt),B

def simulate3(lot,a,pair,bc=0,bcc=0,sc=0,scc=0):
    balance = 0
    ol = 0
    l = 0
    maxB = -999999
    ...
    tg = 0     # total gains count
    for i in range(n-1):
        if B[i+1]>B[i]: tg += 1
        elif B[i+1]<B[i]: tg -= 1
    return tg/n, (-100/-maxDD), float(pcnt)/cnt,tcnt,fdp(float(positive)/cnt),fdp(float(rpositive)/cnt),B

def simulate9(lots,a,p,f,omx):
    print("simulate 9 begins..."+f)
    mx = -999999999
    bc = 0
    bcc = 0
    sc = 0
    ...
    bm = 0
    debug = False
    factor = (max(lots)-min(lots))/2/1000
    ...
    if mx > omx:            
        ...
        plt.plot(np.array(range(len(bB))), np.array(bB), label= '3x')
        plt.savefig(d+"/"+f+"_testing_balance.png")
        plt.close()                                            
        print(p+" best tresholds: "+str(round((bc-sc)/2/factor/10,1))+"%  :  \t"+str(bc)+"\t"+str(bcc)+"\t"+str(sc)+"\t"+str(scc))
        #print(p+" best tresholds: "+str(round(mx,5))+"  :  \t"+str(bt))
        w = open(d+"/"+f+".index",'w')
        w.write(str(bc)+","+str(bcc)+","+str(sc)+","+str(scc)+","+str(round(bm,5))+","+str(round(mx,3)))
        w.close()
    return mx,bm,(bc-sc)/2,0,0,0,B

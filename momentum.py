...
def prepare(p,tf,isTrender=False):
    ...
    while True:
        ...
        for period in range(10,2000,10):
...
def gb_train(s,c):
    xf,yf,ts,x,y = pload("data/"+c+str(tf)+".s"+str(s))
    xx, xt, yy, yt = train_test_split(x[:-validation],y[:-validation],test_size=0.05, random_state=1688, shuffle=False)
    print(c+" "+str(s)+": Train Test Split X_33333 Veritication: " + str(xx[333][0])+"   Y:"+str(yy[333]))
    lr = 0.2
    best_score=-9999
    best_regressor=None
    LV = randint(50,500)
    SP = randint(2,13)
    SS = uniform(0.3,0.9)
    r=GradientBoostingRegressor(verbose=0, warm_start=True, loss='huber', learning_rate=lr, n_estimators=1,max_depth=8,min_samples_leaf=LV,min_samples_split=SP,subsample=SS)
    ...
    for j in range(1,3000):
        ...
        r.fit(xx,yy)
        score = my_score(yt,r.predict(xt))
        if score>best_score:
            bj=j
            best_score = score
...
def get_very_best_models():
    ...
    for c in exotics:
        ...
        mt = -1
        for t in [5]:
            xf,yf,ts,x,y = pload("data/"+c+str(t)+".s"+str(5))
            ts=ts[-testing:]
            x = x[-testing:]
    ...
    if argv[1]=="degap":
        ...
        for shift in range(5,120,5):
            for p in ["EURUSD"]:
                ...
                xx[shift][p], xt[shift][p], yy[shift][p], yt[shift][p] = train_test_split(x[:-validation],y[:-validation],test_size=0.05, random_state=1688, shuffle=False)
        ...
        pool = mp.Pool(7)
        for p in ["EURUSD"]:
            xf,yf,ts,x,y = pload("data/"+p+str(tf)+".s5")
            ts = ts[-testing:]
            x = x[-testing:]

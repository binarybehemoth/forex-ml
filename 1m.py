...
tf = 1
month = int(60/tf*24*23)    # months to test
iterations = 88
boundary = 0.38
starting   = 60
train_period = 12
...
def rf_train(start,p):
    ...
    outfile = p+"."+str(start)+"_"+str(LV)+"_"+str(SP)+"_"+str(SS)+".regressor"
    print("training "+p+str(s)+"------------------->"+d+"/"+outfile)
    bj=0
    bm=0
    for j in range(100,101,30):
        r.set_params(n_estimators=j)
        r.fit(xx[(-starting+start)*month:(-starting+start+train_period)*month],yy[0][p][(-starting+start)*month:(-starting+start+train_period)*month])
        score,m,bt,cnt,pp,rpp,B = obtain_threshold2(p,r,r.predict(xt[(-starting+start+training_period)*month:(-starting+start+train_period+validation)*month]),a[p][(-starting+start+training_period)*month:(-starting+start+train_period+validation)*month],p)
...
def obtain_threshold2(p,m,x,a,outfile):
    a = []
    ...
    for j in range(1,10000):
        t = j / factor
        balance,m,wRate,cnt,pp,rpp,B = simulate(y,a,p,t,0,-t,0)
    ...
    for j in range(1,3000):
        t = 0 - j / factor
        balance,m,wRate,cnt,pp,rpp,B = simulate(y,a,p,bc,t,sc,-t)
        if (debug): print(p+":"+str(t)+":   "+str(round(balance*m,2))+":"+str(cnt)+":"+str(round(m,3)))
        if balance*m>mx:
            mx = balance*m
            bcc= t
    ...
    for j in range(1,10000):
        t = 0 - j / factor
        balance,m,wRate,cnt,pp,rpp,B = simulate(y,a,p,9,0,t,0)
        if (debug): print(p+":"+str(t)+":   "+str(round(balance*m,2))+":"+str(cnt)+":"+str(round(m,3)))
        if cnt<20:break
        if balance*m>mx:
            mx = balance*m
    ...
    for j in range(1,3000):
        t = j / factor
        balance,m,wRate,cnt,pp,rpp,B = simulate(y,a,p,bc,bcc,sc,t)
        if (debug): print(p+":"+str(t)+":   "+str(round(balance*m,2))+":"+str(cnt)+":"+str(round(m,3)))
        if balance*m>mx:
            mx = balance*m
            scc= t
...
if __name__ == '__main__':
    ...
    if argv[1]=="test_i":
        ...
        for iter in range(iterations):
            for start in range(starting-train_period-validation-testing):
                if "JPY" in p: continue
                #pl.append(pool.apply_async(gb_train, (shift,p)))
                pl.append(pool.apply_async(rf_train, (start,p)))
                #nn_train(shift,p)
                #xg_train(shift,p)
        for pl2 in pl: pl2.get()
